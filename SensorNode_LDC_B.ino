...
#define CAPTURE_TIME_SAMPLES            MAX_SAMPLES_COMPRESSED_AMOUNT1 * 100
#define COM_AND_TRANS_TIME_SAMPLES      100
...
{  
  ...
  if (Serial.available() > 0)
  {
    ...
    {     
      ...
      //captura de tiempo.      
      capture_time[capture_time_counter] = intial_time - millis();
      ...
      { 
        ...
        for(int i1 = 0; i1 < 9;++i1)
        {
          ...
          ldc.initialization(bits_resolution_data1[i1], MAX_SAMPLES_COMPRESSED_AMOUNT2, MAX_PAYLOAD_SIZE, data_set1[i1], MAX_SAMPLES_COMPRESSED_AMOUNT1);

          /*//obtención del máximo de di y code en tabla.
          max_di1   = ldc.maxTable(0);
          max_code1 = ldc.maxTable(2);
          ...
          ldc.joinBits(ldc.getLength_fdi(), 8, stream_com_data, stream_index_available, stream_available_bits);
          ldc.joinBits(di_bits, 8, stream_com_data, stream_index_available, stream_available_bits);
          ldc.joinBits(code_bits, 8, stream_com_data, stream_index_available, stream_available_bits);
    
          //bucle para salvar la tabla.
          for (int i2 = 0; i2 < ldc.getLength_fdi(); i2++)
          {
            //concatenación de bits para tabla.
            ldc.joinBits(ldc.getTable(i2, 0), di_bits, stream_com_data, stream_index_available, stream_available_bits);        
            ldc.joinBits(ldc.getTable(i2, 1), code_bits, stream_com_data, stream_index_available, stream_available_bits);
            ldc.joinBits(ldc.getTable(i2, 2), 8, stream_com_data, stream_index_available, stream_available_bits);
          }*/

          //compresión de muestras.
          compress_size = ldc.compressorLDC(data_set1[i1], samples_compressed_amount1, bits_resolution_data1[i1], stream_com_data, stream_index_available, stream_available_bits); 
        }
        
        //captura de tiempo.      
        compress_time[com_and_trans_time_counter]     = intial_time - millis();
        ...
        //captura de tiempo.      
        transmition_time[com_and_trans_time_counter]  = intial_time - millis();
